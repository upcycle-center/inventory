...
export default async function RequestPage({ searchParams }: { searchParams: { location?: string } }) {
  ...
  const [{ data: locations }, { data: locationProducts }, draft] = await Promise.all([
    ...
    supabase
      ...
      .select(
        "location_id, product:products(id, sku, description, photo_url, active, case_size, middle_unit_label, middle_unit_size, each_countable), storage_area:storage_areas(id, code, name)"
      )
      .eq("active", true),
